...
Comportamiento:
- Interactivo: seleccionar con el ratón una región en el panel superior para que
  el panel inferior muestre el zoom.
- Al cerrar la ventana, si se seleccionó un zoom, se guardará una imagen del zoom.
"""
...
# --- Asegurar que 'src' esté en sys.path (ajusta según tu estructura si main.py no está en la raíz) ---
base_dir = os.path.dirname(os.path.abspath(__file__))
src_dir = os.path.join(base_dir, "src")
if os.path.isdir(src_dir) and src_dir not in sys.path:
    sys.path.insert(0, src_dir)

# Intentar usar backend Qt si PyQt5 está instalado (necesario para widgets interactivos)
import matplotlib
try:
    import PyQt5  # si esto falla, no forzamos backend
    if not matplotlib.get_backend().lower().startswith('qt'):
        matplotlib.use('Qt5Agg')  # debe hacerse antes de importar pyplot
except Exception:
    pass
...
# Importaciones del paquete (si dan error, verás el traceback)
try:
    ...
    traceback.print_exc()
    # Definir sustitutos mínimos para que el archivo falle de forma controlada
...
def plot_spectrum_with_analysis(wavelengths, flux_original, flux_processed, lines_dict, report):
    ...
    Visualización interactiva:
     - Panel superior: espectro completo (seleccionar rango con el ratón).
     - Panel inferior: zoom de la selección.
    Devuelve: (fig, selected_span) donde selected_span es dict {'xmin', 'xmax'} o None si no se seleccionó.
    """
    ...
    # contenedor para la última selección
    selected_span = {'xmin': None, 'xmax': None}

    def onselect(xmin, xmax):
        # Normalizar límites por si el usuario arrastró de derecha a izquierda
        if xmax < xmin:
            xmin, xmax = xmax, xmin
    ...
    # SpanSelector: selection with mouse on ax_full
...
def analyze_file(fits_file):
    ...
    try:
        ...
        # Crear figura interactiva con selector
        fig, selected_span = plot_spectrum_with_analysis(wl_r, flux_r, flux_plot, lines_dict, report)

        # Mostrar ventana interactiva (aquí el usuario hace la selección)
        plt.show()  # bloquea hasta que el usuario cierre la ventana

        # Guardado: si el usuario seleccionó un zoom, guardamos imagen del zoom,
        # si no, guardamos la figura completa.
        xmin = selected_span.get('xmin')
        xmax = selected_span.get('xmax')
        if xmin is not None and xmax is not None:
            mask = (wl_r >= xmin) & (wl_r <= xmax)
            if np.any(mask):
                # crear figura para zoom y guardar
        ...
        print("\n✅ Análisis completado. Archivos guardados (si procede):")
        print("- spectral_analysis_report.json")
        print("- processed_spectrum.csv")
        print("- detailed_spectral_analysis(_zoom).png")
...
def main():
    ...
    if args.file:
        ...
        fits_file = input("Introduce la ruta del archivo FITS: ").strip().strip('"')
